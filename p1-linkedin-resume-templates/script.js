...
const renderTemplates = () => {
    ...
    document.getElementById('tpl-1').innerHTML = `
        <div class="tpl-modern">
            <div class="header">
                ...
                <div style="font-size: 0.6em">${userData.contact.email} | ${userData.place}</div>
            </div>
            <div class="content">
                <section>
                ...
                <section>
                    ...
                    <p style="font-size: 0.7em">${userData.skills.join(', ')}</p>
                </section>
                <section>
                    <div class="section-title">Skills</div>
                    <p style="font-size: 0.7em">${userData.languages.join(', ')}</p>
                </section>
            </div>
        </div>
    `;
    ...
    document.getElementById('tpl-2').innerHTML = `
        <div class="tpl-creative">
            <div class="sidebar">
                ...
                <div style="font-size: 0.6em; margin-bottom: 10px;">${userData.place}<br>${userData.contact.phone}</div>
    ...
    document.getElementById('tpl-3').innerHTML = `
        <div class="tpl-tech">
            ...
            <p style="font-size: 0.7em; margin-bottom: 15px; color: #94a3b8;">${userData.summary}</p>
            <div class="accent" style="margin-bottom: 5px;">[Experience]</div>
            ${userData.experience.map(exp => `
                <div class="experience-item">
                    <div style="color: #f1f5f9">${exp.role}</div>
                    <div style="font-size: 0.6em">${exp.company} | ${exp.period}</div>
                </div>
            `).join('')}
            <div class="accent" style="margin-top: 10px;">[Contact]</div>
            <div style="font-size: 0.6em;">PLACE: ${userData.place}</div>
            <div style="font-size: 0.6em;">EMAIL: ${userData.contact.email}</div>
            <div style="font-size: 0.6em;">PHONE: ${userData.contact.phone}</div>
            <div class="accent" style="margin-top: 10px;">[Skills]</div>
            <div style="font-size: 0.6em;">${userData.skills}</div>
            <div class="accent" style="margin-top: 10px;">[Languages]</div>
            <div style="font-size: 0.6em;">${userData.languages}</div>
        </div>
    `;
    ...
    document.getElementById('tpl-4').innerHTML = `
        <div class="tpl-bold">
            <div class="sidebar">
                <div style="font-size: 1.2em; font-weight: bold;">${userData.fullName.split(' ')[0]}</div>
                <div style="margin-bottom: 20px;">${userData.fullName.split(' ')[1]}</div>
                <small>CONTACT</small>
                <div style="font-size: 0.5em; color: white; margin-bottom: 16px;">${userData.place}<br>${userData.contact.email}<br>${userData.contact.phone}</div>
                <small>SKILLS</small>
                <div style="font-size: 0.6em; color: white;margin-bottom: 20px;">${userData.skills.join('<br>')}</div>
                <small>LANGUAGES</small>
                <div style="font-size: 0.6em; color: white;">${userData.languages.join('<br>')}</div>
            </div>
            <div class="main">
                <div style="font-weight: bold; text-transform: uppercase; letter-spacing: 1px;">Profile</div>
                <p style="font-size: 0.7em; margin: 10px 0;">${userData.summary}</p>
    ...
    document.getElementById('tpl-5').innerHTML = `
        <div class="tpl-elegant">
            ...
            <p style="font-style: italic; text-align: center; font-size: 0.7em; margin-bottom: 15px;">${userData.tagline}</p>
            <div style="text-align: center; font-weight: bold; text-decoration: underline; margin-bottom: 5px;">PROFESSIONAL SUMMARY</div>
            <p style="font-size: 0.7em; text-align: justify; margin-bottom: 15px;">${userData.summary}</p>
            ...
            <p style="font-size: 0.6em; text-align: center; margin-bottom: 15px;">${userData.skills}</p>
            <div style="text-align: center; font-weight: bold; text-decoration: underline; margin-bottom: 5px;">LANGUAGES</div>
            <p style="font-size: 0.6em; text-align: center; margin-bottom: 15px;">${userData.languages}</p>
        </div>
    `;
    ...
    <div class="tpl-slate">
        <div class="top-bar">
            ...
            <div style="text-align:right; font-size:0.5em">${userData.place}<br>${userData.contact.email}<br>${userData.contact.phone}</div>
        </div>
        <div class="main-grid">
            <div style="grid-column: span 2;"><p class="summary-text" style="font-size:0.65em">${userData.summary}</p></div>
            <div style="border-right:1px solid #eee; padding-right:10px">
                <small><b>EXPERIENCE</b></small>
                ${userData.experience.map(e => `<div style="font-size:0.6em; margin-top:5px"><b>${e.company}</b><br>${e.role}</div>`).join('')}
            </div>
            <div>
                <small><b>SKILLS</b></small>
                <div style="font-size:0.6em; margin-top:5px">${userData.skills.join('<br>')}</div>
            </div>
            <div>
                <small><b>LANGUAGES</b></small>
                <div style="font-size:0.6em; margin-top:5px">${userData.languages.join('<br>')}</div>
            </div>
        </div>
    </div>
`;
    ...
    <div class="tpl-royal">
        ...
        <p style="font-size:0.65em; margin:10px 0; text-align:center">${userData.summary}</p>
        <div style="font-weight:bold; font-size:0.7em; color:#b7791f; margin-top:16px">EXPERIENCE</div>
        <div style="display:flex; flex-wrap:wrap; gap:5px; margin-top:5px">
            ${userData.experience.map(s => `<span style="border:1px solid #b7791f; padding:2px 4px; font-size:0.5em">${s.company} - ${s.role}</span>`).join('')}
        </div>
        <div style="font-weight:bold; font-size:0.7em; color:#b7791f; margin-top:16px">CORE EXPERTISE</div>
        <div style="display:flex; flex-wrap:wrap; gap:5px; margin-top:5px">
            ${userData.skills.map(s => `<span style="border:1px solid #b7791f; padding:2px 4px; font-size:0.5em">${s}</span>`).join('')}
        </div>
        <div style="font-weight:bold; font-size:0.7em; color:#b7791f; margin-top:16px">LANGUAGES</div>
        <div style="display:flex; flex-wrap:wrap; gap:5px; margin-top:5px">
            ${userData.languages.map(s => `<span style="border:1px solid #b7791f; padding:2px 4px; font-size:0.5em">${s}</span>`).join('')}
        </div>
    </div>
`;
    ...
    <div class="tpl-bordered">
        ...
        <div style="font-size:0.7em; margin-bottom:10px">${userData.tagline}</div>
        <hr style="border:0; border-top:1px solid #2d3748; margin:8px 0">
        <p style="font-size:0.6em; margin:16px 0; line-height:1.2"><b>Profile:</b> ${userData.summary}</p>
        <div style="display:grid; grid-template-columns: 1fr; gap:16px">
            <div style="font-size:0.6em"><b>Contact:</b><br>${userData.place}<br>${userData.contact.email}</div>
            <div style="font-size:0.6em"><b>Experience:</b><br>${userData.experience.map(e => `${e.company} - ${e.role}<br>`).join('')}</div>
            <div style="font-size:0.6em"><b>Skills:</b><br>${userData.skills.map(lang => `${lang}`)}</div>
            <div style="font-size:0.6em"><b>Languages:</b><br>${userData.languages.map(lang => `${lang}`)}</div>
        </div>
    </div>
`;
    ...
    <div class="tpl-duo">
        <div class="left">
            <div style="font-weight:bold; font-size:0.7em">SKILLS</div>
            <div style="font-size:0.55em; margin-top:5px">${userData.skills.join('<br>')}</div>
            <div style="font-weight:bold; font-size:0.7em; margin-top:15px">LANGS</div>
            <div style="font-size:0.55em">${userData.languages.join(', ')}</div>
        </div>
        <div class="right">
            <h3 style="color:#2f855a">${userData.fullName}</h3>
            <p style="font-size:0.6em; margin:8px 0">${userData.summary}</p>
            <div style="background:#f0fff4; padding:5px; font-size:0.6em; margin:16px 0">
...
`;
    ...
    <div class="tpl-noir">
        ...
        <p style="font-size:0.65em; color:#aaa; margin-bottom:15px">${userData.summary}</p>
